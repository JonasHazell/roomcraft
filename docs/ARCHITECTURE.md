...
| **Multi-select furniture** — shift/ctrl/cmd-click (desktop) or the selection bar's "Select multiple" toggle (touch) to group 2+ pieces; group move/duplicate/delete (#370) | `panel/SelectionBar.tsx`, `scene/FurnitureMesh.tsx` | `useUiStore` (`furniture-multi` selection, `multiSelectMode`) | `store/selectors.ts` (`useSelectedFurnitureIds`), `lib/globalKeydown.ts` |
| **Product link / "Buy"** — attach a real purchase link (+ optional price/retailer) to a placed piece (#351, first step toward the furniture-catalogue revenue model) | `panel/FurnitureFields.tsx` (`ProductLinkFields`), `panel/FurnitureDialog.tsx` (footer "Buy" button) | `slices/furnitureSlice.ts` (`FurnitureItem.product`) | `types.ts` (`FurnitureProduct`, `isHttpsUrl`), `lib/persistence.ts` (`normalizeProduct`) |
...
| **Shareable link** — read-only, point-in-time snapshot link for a room, the first collaboration step (#353) | `panel/ProposalSwitcher.tsx` ("Share"), `panel/ShareDialog.tsx`, `share/ShareView.tsx`, `share/ShareScene.tsx` | `useShareStore.ts` | `lib/shareApi.ts` → `server/share.ts` |
