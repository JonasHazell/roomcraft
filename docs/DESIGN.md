...
| `--accent`        | Primary action, focus, active accents (indigo)   |
...
hairlines (`--line`). The accent is a modern indigo. `--select` stays the
canvas selection blue (3D/plan only) and is the only blue allowed in the
canvas — keep chrome accents on `--accent`. Score/severity bands use their own
fixed greens/ambers/reds (see `.score-*`, `.severity-*`).
