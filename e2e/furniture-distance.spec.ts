 ...
 * properties panel shows a read-only "N cm to nearest piece" line — the
 * touch-friendly, relative-measurement alternative to exact X/Z position fields
 * the human asked for after rejecting #153 (see docs/AGENT_LEARNINGS.md). Runs
 * in both the `desktop` and `mobile` projects.
 *
...
test('shows distance to nearest piece when a second piece is selected', async ({ page }) => {
  ...
  // Add the first piece: with no other furniture yet, no distance shows.
  ...
  // Commit the sofa, then add a second piece — now the nearest-piece distance shows.
  await page.getByRole('button', { name: 'OK', exact: true }).click();
  await page.getByRole('button', { name: 'Add furniture' }).click();
  await page.getByRole('button', { name: 'Chair', exact: true }).click();

  await expect(page.getByRole('dialog', { name: 'Furniture settings' })).toBeVisible();
  const distanceField = page.locator('.field-static').filter({ hasText: /nearest piece/i });
  await expect(distanceField).toBeVisible();
  await expect(distanceField).toHaveText(/^\d[\d\s]* cm to nearest piece$/);
});
